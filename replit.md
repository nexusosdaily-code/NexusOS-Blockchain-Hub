...
- **Governance & Voting**: Features a Civic Governance campaign system for validator innovation, with AI-powered dispute resolution (`ai_arbitration_controller.py`) and AI-assisted governance decisions (`nexus_ai_governance.py`). The **NexusOS Constitution v1** (`governance/constitution.json`) defines 3 core clauses: Non-Dominance (no entity >5% authority without PLANCK consensus), Immutable Rights (basic rights protected at YOCTO level), and Energy-Backed Validity (system actions require energy escrow). The **Constitutional Enforcer** (`governance/enforcer.py`) automatically validates all governance frames against these clauses.
