...
st.set_page_config(page_title="WNSP v4.0 Quantum Consensus", layout="wide")
...
if "qec" not in st.session_state:
    st.session_state.qec = None

# Sidebar configuration
with st.sidebar:
    st.header("⚙️ Configuration")
    num_validators = st.slider("Number of Validators", 3, 10, 5)
    consensus_threshold = st.slider("Consensus Threshold", 0.5, 1.0, 0.67, 0.05)
    
    if st.button("🔧 Initialize Quantum Network", type="primary"):
        st.session_state.validators = [
            QuantumValidator(f"validator_{i}", EPRPair(f"pair_{i}", 0, 0))
            for i in range(num_validators)
        ]
        st.session_state.qec = QuantumEnergyAwareConsensus(
            st.session_state.validators,
            threshold=consensus_threshold
        )
        st.session_state.qec.distribute_epr_pairs()
        st.session_state.consensus_history = []
...
# TAB 1: Live Consensus Simulation
with tab1:
    st.subheader("Real-Time Transaction Validation")
    
    if st.session_state.qec is None:
        st.warning("⚠️ Initialize quantum network first using the sidebar")
    else:
        ...
        with col3:
            amount = st.number_input("Amount (NXT)", 1.0, 10000.0, 10.5)
        
        if st.button("⚛️ Validate Transaction", type="primary"):
            tx = Transaction(
            ...
            # Validate
            is_valid, record = st.session_state.qec.validate_with_energy_awareness(tx)
            st.session_state.consensus_history.append(record)
...
with tab2:
    ...
    # Consensus history visualization
    if st.session_state.consensus_history:
        st.subheader("Bell Violation Over Time")
        
        bell_violations = [h['bell_violation'] for h in st.session_state.consensus_history]
...
with tab3:
    ...
    if st.session_state.qec is None:
        ...
        with col2:
            if st.button("🔍 Run Byzantine Detection", type="secondary"):
                byzantine = st.session_state.qec.detect_byzantine_nodes()
        ...
        # Validator reliability chart
        if st.session_state.consensus_history:
            st.subheader("Validator Reliability Score")
            
            validator_scores = {}
            for record in st.session_state.consensus_history:
                for m in record['measurements']:
            ...
            for val_id, measurements in validator_scores.items():
                ...
                reliability = abs(0.5 - consistency) * 100  # How far from random (50%)
...
with tab4:
    ...
    **Quantum Energy Integration**:
    ...
    if st.session_state.consensus_history:
        ...
        total_energy_costs = [h['total_energy_nxt'] for h in st.session_state.consensus_history]
        ...
        with col3:
            st.metric("Transactions", len(st.session_state.consensus_history))
        ...
        wavelength_data = []
        for val_id, wavelength in st.session_state.qec.wavelengths.items():
            color_map = {
                400: "Violet",
                450: "Blue",
                500: "Cyan",
...
**Documentation**: See `WNSP_v4_Integration_Guide.md`  
**Source Code**: `wnsp_quantum_entanglement_poc.py`
""")
